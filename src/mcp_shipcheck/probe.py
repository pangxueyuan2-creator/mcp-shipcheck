...
# Versions this client actually understands. An initialize response outside
# this set is an explicit incompatibility, never a silent success.
...
def probe_command(command: Iterable[str], timeout: float = 5.0) -> dict[str, Any]:
    ...
    try:
        ...
        if protocol_version not in SUPPORTED_PROTOCOL_VERSIONS:
            ...
            raise ProtocolError(
                f"server protocolVersion {protocol_version!r} is not supported "
